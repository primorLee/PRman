...
3. Collect observable evidence for the proposed change. Never infer a passing gate from scorer output
   or from the absence of an error.
...
5. Run the deterministic helper from this skill directory:
   ...
   The helper evaluates supplied evidence only; it does not run tests, edit code, or call GitHub.
...
The scorer is optional and replaceable. Without a configured production scorer, passing hard gates
still results in `abstain`; never substitute a fixture or static scorer. Read
